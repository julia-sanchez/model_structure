...
const int pixel_radius_for_growing = 10;                                        // in case of soft obstruction over all the length  of the plane OR if black pixels (no return of laser bim)
...
const double normals_similarity_threshold_to_select_seed = 0.95;                 // comparison of seed normal to neighbors normals (seed must be on a precise zone of a plane)
...
public:
    ...
    manager(typename pcl::PointCloud<pcl::PointNormal>::Ptr c, int Nr, int Nc, double pa, double ta, int mc);
...
//    ----------------------------------------------------------------------------------------------------------------------------------------
    ...
    void initSearchCluster(double rad, Eigen::Vector3d rot_axis);
...
private:
    ...
    std::map<std::pair<int,int>, std::pair<Eigen::Vector3d, Eigen::Vector3d>> XY2PtN;   // X Y
    //we associate one mode plane to each point
    std::multimap<std::pair<int,int>, Eigen::Vector3d> XY2Plane_clusterized; // to keep double values
    ...
    std::vector<std::vector<intersection>> edges;
    ...
    std::vector<std::map<std::pair<int,int>, std::pair<Eigen::Vector3d, Eigen::Vector3d>>::iterator> seeds;
    ...
    void computeTheoriticalLinesIntersections(std::vector<std::vector<intersection>> vec_openings);
